...
//	SpiInstance.SlaveSelectReg = 0;
	...
	if (XSpi_Transfer(&SpiInstance, wr_data, NULL, 2) != 0){
		print("SPI device failed to write register");
	}
...
//	SpiInstance.SlaveSelectReg = 0;
	...
	if (XSpi_Transfer(&SpiInstance, wr_data, NULL, nbytes+1) != 0){
		print("SPI device failed to write bytes");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
	return (data+nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE w_data[2] = {reg, 0};
	BYTE r_data[2] = {0, 0};
	if (XSpi_Transfer(&SpiInstance, w_data, r_data, 2) != 0){
		print("SPI device failed to read register");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
	return r_data[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	BYTE w_data[nbytes+1];
	BYTE r_data[nbytes+1];
	w_data[0] = reg;
	if (XSpi_Transfer(&SpiInstance, w_data, r_data, nbytes+1) != 0){
		print("SPI device failed to read bytes");
	}
	for (int i = 0; i < nbytes; i++){
		data[i] = r_data[i+1];
	}
